...
public class SearchDormitoryPanel extends JFrame {
	...
	// public void run() {
	// try {
	// SearchDormitoryPanel frame = new SearchDormitoryPanel();
	 ...
	 * Create the frame.
	 */
	public SearchDormitoryPanel(DormitoryInfoPanel dip) {
		...
		search.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (building.getText().equals("") || room.getText().equals("")) {
					...
					JOptionPane.showMessageDialog(null, "该宿舍不存在,请重新输入", "消息提示", JOptionPane.WARNING_MESSAGE);
					building.setText("");
					room.setText("");
				} else {
					//
					...
					// dispose();
					dip.change();
					dip.showTable(admin.searchDormitory(building.getText(), room.getText()));
				}
			}
		});
		...
		gl_contentPane.setVerticalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING).addGroup(gl_contentPane
				...
				.addGap(44).addComponent(search).addContainerGap(47, Short.MAX_VALUE)));
